...
export class ProductService {
  ...
  async read(page: number = 0, size: number = 5, successCallBack?: () => void, errorCallBack?: (errorMessage: string) => void): Promise<{ totalCount: number; products: List_Product[] }> {
    try {
      ...
      throw errorResponse; // Re-throw the error to propagate it further if needed.
    }
  }
  ...
  //   string)=>void ):Promise<{totalCount:number;
  //     products:List_Product[]}>{
  //  const promiseData:Promise<{totalCount:number;
  //   products:List_Product[]}>= this.httpClientService.get<{totalCount:number;
  // products:List_Product[]}>({
  //     controller:"products",
  //     queryString:`page=${page}&size=${size}`
  //   }).toPromise()
